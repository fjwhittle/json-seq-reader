 ...
 */
async function* jsonSeqReader (rs, progressCallback, RS = 0x1e) {
  ...
  while(!done && ({ done, value } = await reader.read())) {
    if(value) {
      ...
      let newarr = new Uint8Array(all.length + value.length);
      newarr.set(all);
      ...
      while(idx = all.indexOf(RS), idx > -1 || done) {
        ...
        let nextSlice = idx > 0 ? all.subarray(0, idx - 1) : all;
        let nextChunk = utf8Decoder.decode(nextSlice);
